...
static void canzero_serialize_canzero_message_power_board12_stream_errors(canzero_message_power_board12_stream_errors* msg, canzero_frame* frame) {
  ...
  ((uint32_t*)data)[0] |= (uint8_t)(msg->m_error_heartbeat_miss & (0xFF >> (8 - 1))) << 2;
  ((uint32_t*)data)[0] |= (uint8_t)(msg->m_error_level_mcu_temperature & (0xFF >> (8 - 2))) << 3;
}
...
static void canzero_deserialize_canzero_message_mother_board_stream_pdu_12v_command(canzero_frame* frame, canzero_message_mother_board_stream_pdu_12v_command* msg) {
  uint8_t* data = frame->data;
  msg->m_power_board12_command = (pdu_12v_command)(((uint32_t*)data)[0] & (0xFFFFFFFF >> (32 - 2)));
}
...
static PROGMEM void canzero_handle_get_req(canzero_frame* frame) {
  ...
  case 27: {
    ...
    resp.m_header.m_eof = 0;
    resp.m_header.m_toggle = 0;
    schedule_get_resp_fragmentation_job(__oe_error_level_config_mcu_temperature_rx_fragmentation_buffer, 7, 27, msg.m_header.m_client_id);
    break;
  }
  case 28: {
...
void canzero_init() {
  __oe_config_hash = 11359940042916041400ull;
  __oe_build_time = {
    .m_year = BUILD_YEAR,
    .m_month = BUILD_MONTH,
    .m_day = BUILD_DAY,
